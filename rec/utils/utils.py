...
def get_parameter_sizes(model: nn.Module):
    ...
    for name, module in model.named_modules():
        exclude = any(name_ in name for name_ in ['time_encoder', 'ID_embedding', 'projection_layer'])
    ...
    # return sum([p.numel() for p in model.parameters() if p.requires_grad and not isinstance(p, nn.Embedding)])
...
class NegativeEdgeSampler(object):
    ...
    def sample(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
               current_batch_start_time: float = 0.0, current_batch_end_time: float = 0.0, return_indice = False):
        """
        ...
        """
        if self.negative_sample_strategy == 'random':
            negative_src_node_ids, negative_dst_node_ids = self.random_sample(size=size, return_indice=return_indice)
            ...
            negative_src_node_ids, negative_dst_node_ids = self.historical_sample(size=size, batch_src_node_ids=batch_src_node_ids,
                                                                                  ...
                                                                                  current_batch_end_time=current_batch_end_time, return_indice=return_indice)
    ...
    def random_sample(self, size: int, return_indice=False):
        """
        ...
        if return_indice:
            return random_sample_edge_src_node_indices, random_sample_edge_dst_node_indices
        else:
    ...
    def historical_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
                          current_batch_start_time: float, current_batch_end_time: float, return_indice=False):
        """
