...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='Googlemap_CT',
                        choices=['Amazon_books', 'Amazon_elec', 'Amazon_movies', 'Enron', 'GDELT', 'Googlemap_CT', 'ICEWS1819', 'Stack_elec', 'Stack_english', 'Stack_ubuntu', 'Yelp'])
    parser.add_argument('--batch_size', type=int, default=256, help='batch size')
    parser.add_argument('--model_name', type=str, default='SASRec', help='name of the model',
                        choices=[
                            'SASRec', 'HSTU',
                            'SLA', 'GLA', 'MoM', 'GSA', 'MAMBA2', 'ATLAS', 'GDeltanet', 'TTT', 'TTTv2', 'TITANS',
                            'prism', 'prism_ablate_l1', 'prism_ablate_no_nonlinear',
                            'prism_ablate_no_shortconv', 'prism_ablate_no_gain', 'prism_hybrid4'
                        ])
    ...
    parser.add_argument('--num_experts', type=int, default=3, help='number of experts or refine steps')
    ...
    parser.add_argument('--load_best_configs', action='store_false', default=False, help='whether to load the best configurations')
    parser.add_argument('--nan_debug', action='store_true', default=False, help='enable NaN/Inf debug checks')
    parser.add_argument('--ttt_mini_batch_size', type=int, default=16, help='mini-batch/chunk size for TTTv2')
    parser.add_argument('--ttt_base_lr', type=float, default=1.0, help='base inner-loop LR for TTTv2')
    parser.add_argument('--ttt_conv_width', type=int, default=4, help='depthwise conv width for TTTv2')
    parser.add_argument('--ttt_pre_conv', type=int, default=1, choices=[0, 1], help='whether to enable the official pre-conv block in TTTv2')
    parser.add_argument('--ttt_intermediate_factor', type=float, default=8.0 / 3.0, help='SwiGLU expansion factor for TTTv2')
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    :return:
    """
    # model specific settings (only for kept models: SASRec, HSTU, SLA, GLA, MoM, GSA, MAMBA2, ATLAS, GDeltanet, TTT, TITANS, prism)
    if args.model_name in [
        'SASRec', 'HSTU', 'SLA', 'GLA', 'MoM', 'GSA', 'ATLAS', 'GDeltanet', 'TTT', 'TTTv2', 'TITANS',
...
def get_edge_classification_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--model_name', type=str, default='SASRec', help='name of the model',
                        choices=[
                            'SASRec', 'HSTU', 'SLA', 'GLA', 'MoM', 'GSA', 'MAMBA2', 'ATLAS', 'GDeltanet', 'TTT', 'TTTv2', 'TITANS',
                            'prism', 'prism_ablate_l1', 'prism_ablate_no_nonlinear',
                            'prism_ablate_no_shortconv', 'prism_ablate_no_gain', 'prism_hybrid4'
                        ])
    parser.add_argument('--gpu', type=int, default=0, help='number of gpu to use')
