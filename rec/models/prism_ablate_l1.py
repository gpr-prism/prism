"""
PRISM (Ablation RQ3): w/o Iterative Refinement (L=1)
Paper ablation: remove multi-step refinement and keep a single-step Rank-1 update.

1) Write-Forget Decoupling
   S_t = A_t S_{t-1} + B_t                               (Eq. 8)
   A_t: linear forgetting (state-independent, parallel prefix-scan)
   B_t: single-step write (Rank-1 injection)
...
3) Rank-L accumulation: here L is fixed to 1, degenerating to a single Rank-1 outer product
"""
import math
import numpy as np
import torch
...
class PRISM(nn.Module):
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        """
        PRISM model (ablation: w/o Iterative Refinement, fixed L=1)
        - Forget path: linear, state-independent recurrence A_t (parallel prefix-scan; Eq. 7/8)
        - Write path: input-anchored + single-step injection B_t (degenerates to Rank-1)
        """
        ...
        self.device = device
        # Ablation setting: fix L=1, no multi-step iterative refinement
        self.solver_steps = 1
    ...
    def compute_src_node_temporal_embeddings(self, src_node_ids: np.ndarray,
        ...
        for prism_block in self.PRISM_blocks:
            # Sequence update block corresponding to "input-anchored + single-step injection" in the paper
...
class PRISMBlock(nn.Module):
    def __init__(self, embedding_dim, linear_hidden_dim, attention_dim, dropout_ratio, num_heads,
        ...
        self._eps = epsilon
        # Ablation: fix L=1 (single-step Rank-1 update)
        self._solver_steps = 1
        ...
        # Stage-2: single-step update (L=1), degenerate form of Eq. 10–16
        ...
        self.step_gate = nn.ModuleList([nn.Linear(self._embedding_dim, self._num_heads)
                                        for _ in range(self._solver_steps)])
        self.step_delta = nn.ModuleList([nn.Sequential(
        ...
        # Maps the "visible updates" of the single-step injection back to the representation space
    ...
    def _compute_prism_injection(self, anchor, gain, gk_scale, gk_head_gate, gk_inject_gate):
        """
        Compute injection term (write path):
        Ablation setting L=1, so B_t degenerates to a single Rank-1 outer product
        """
        ...
        for step in range(self._solver_steps):
            ...
            # Implementation of \\delta_t^{(l)} = GELU(p_t^{(l)} \\odot r_t^{(l)}) (Eq. 15)
            # Here gain is the shared proxy for p_t^{(l)}, residual is r_t^{(l)}
            delta = gate_scalar * (gain * residual)
            if gk_scale is not None:
                # Scale modulation for training stability; does not change the single-step structure
    ...
    def _gated_linear_attention_with_injection(self, q, k, v, gk, injection_k_list, injection_v_list, gk_inject_gate,
        ...
        # Single-step injection (from PRISM solver): degenerate form of Eq. 10/18
    ...
    def forward(self, inputs, timestamps):
        ...
        # PRISM write path: input anchoring + single-step injection (degenerate form of Eq. 9–18)
        ...
        # Final output: linear recurrence backbone + single-step injection branch (Write-Forget Decoupling)
