...
class SLA(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler, num_neighbors: int,
        ...
        self.output_layer_src = nn.Linear(in_features=4*self.embedding_dim, out_features=64, bias=True)
        self.output_layer_dst = nn.Linear(in_features=2*self.embedding_dim, out_features=64, bias=True)
    ...
    def compute_src_node_temporal_embeddings(self, src_node_ids: np.ndarray,
        ...
        for SLA_blocks in self.SLA_blocks:
            # self-attention block
            # Tensor, shape (batch_size, num_neighbors + 1, node_feat_dim)
            src_node_features = SLA_blocks(inputs=src_node_features, timestamps=src_neighbor_times)
