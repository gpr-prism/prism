...
def _load_gated_deltanet():
    try:
        from myfla.gated_deltanet import GatedDeltaNet
    except ModuleNotFoundError as exc:
        if exc.name == "fla":
            raise ModuleNotFoundError(
                "The GDeltanet baseline requires the optional "
                "`flash-linear-attention` package, which provides `fla`. "
                "Install it before running with --model_name GDeltanet."
            ) from exc
        raise
...
class GDeltanet(nn.Module):
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler, num_neighbors: int,
        ...
        GatedDeltaNet = _load_gated_deltanet()
