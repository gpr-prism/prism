"""
PRISM (Ablation RQ3): w/o Gain Predictor
Paper ablation: remove the gain predictor and use a constant step size (fixed update scale).
...
2) Input-Anchored Loop Unrolling
   ...
   Note: in this ablation the gain predictor is removed; the step size is constant.
   for l = 1..L:
...
class PRISM(nn.Module):
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        """
        PRISM model (ablation: w/o Gain Predictor)
...
class PRISMBlock(nn.Module):
    def __init__(self, embedding_dim, linear_hidden_dim, attention_dim, dropout_ratio, num_heads,
        ...
        # Contextual gain predictor:
        # Ablation: remove gain predictor, use a fixed step size (constant gain)
        self.gain_mlp = None
    ...
    def forward(self, inputs, timestamps):
        ...
        # Ablation: fixed step size, no learned gain predictor
        gain = torch.ones_like(anchor)
