...
if __name__ == "__main__":
    ...
    pretrained_model_name = './qwen_emb'
    ...
    for data_set_name in ['Amazon_movies','Googlemap_CT','Yelp']:
        print(data_set_name)
        ...
        entity_text_reader = pd.read_csv('./DyLink_Datasets/' + data_set_name + '/entity_text.csv', chunksize=batch_size)
        ...
        with tqdm(total = num_node) as pbar:
            for batch in entity_text_reader:
                ...
                with torch.no_grad():
                    output = PLM(**encoded_input.to('cuda'),
                           output_hidden_states=True,  # Force output of hidden states
                           use_cache=False,           # Explicitly disable cache
                           return_dict=True)
                    output = output.last_hidden_state.mean(dim=1).cpu()
        ...
        np.save('./DyLink_Datasets/' + data_set_name + '/e_feat_qwen.npy', entity_embeddings)
