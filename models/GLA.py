...
Based on SLA structure: add data-dependent decay gate.
"""
import math
import numpy as np
import torch
...
class GLA(nn.Module):
    ...
    def compute_src_node_temporal_embeddings(self, src_node_ids: np.ndarray,
        ...
        for gla_block in self.GLA_blocks:
            src_node_features = gla_block(inputs=src_node_features, timestamps=src_neighbor_times)
...
class GLABlock(nn.Module):
    ...
    def forward(self, inputs, timestamps):
        ...
        attn_output = self._gated_linear_attention_maybe_from_cache(q, k, v, gk, self._invalid_attn_mask.to(normed_x.device), initial_state)
        g = self._g(normed_x)
