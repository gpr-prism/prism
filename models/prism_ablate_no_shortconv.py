"""
PRISM (Ablation RQ3): w/o ShortConv Anchor
Paper ablation: remove the local ShortConv anchor and use only the current input projection as u_t.
...
2) Input-Anchored Loop Unrolling
   ...
   Note: in this ablation, ShortConv is removed and u_t is approximated by the current input.
   r^{(1)} = v_t - u_t                                   (Eq. 14)
...
class PRISM(nn.Module):
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        """
        PRISM model (ablation: w/o ShortConv Anchor)
...
class PRISMBlock(nn.Module):
    def __init__(self, embedding_dim, linear_hidden_dim, attention_dim, dropout_ratio, num_heads,
        ...
        # Stage-1: input-anchored proxy (ShortConv)
        # Ablation: remove ShortConv anchor; use current input as u_t
    ...
    def _short_conv_anchor(self, x):
        # Ablation: no short convolution, directly return current input as anchor u_t
        # Equivalent to driving updates by the current token projection (removing local history energy)
        return x
    ...
    def forward(self, inputs, timestamps):
        ...
        # Ablation: anchor does not use ShortConv, only the current input
