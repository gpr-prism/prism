...
class EarlyStopping(object):
    ...
    def load_checkpoint(self, model: nn.Module, map_location: str = None, strict: bool = True):
        """
        ...
        state_dict = torch.load(self.save_model_path, map_location=map_location)
        if strict:
            ...
            if skipped:
                self.logger.warning(
                    f"skip {len(skipped)} keys when loading checkpoint (shape mismatch or missing)."
                )
