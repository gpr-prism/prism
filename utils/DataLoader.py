...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float, args):
    ...
    graph_df = pd.read_csv(os.path.join(directory_path, '../DyLink_Datasets/{}/edge_list.csv'.format(dataset_name, dataset_name)))
    ...
    NODE_FEAT_DIM = 768#1024
    EDGE_FEAT_DIM = 768
    if args.use_feature == 'Bert':
        print('get pretrained features')
        edge_raw_features = np.load(os.path.join(directory_path, '../DyLink_Datasets/{}/r_feat.npy'.format(dataset_name, dataset_name)))
        node_raw_features = np.load(os.path.join(directory_path, '../DyLink_Datasets/{}/e_feat.npy'.format(dataset_name, dataset_name)))
...
# Count node frequencies
    node_counts = Counter(src_node_ids)
    
    # Create a set of nodes that appear at least 40 times
    frequent_nodes = {node for node, count in node_counts.items() if count >= 40}
    
    # Filter edges where both nodes are frequent
    mask = np.array([src in frequent_nodes 
    ...
    train_mask = node_interact_times <= val_time#np.logical_and(node_interact_times <= val_time, observed_edges_mask)
    ...
    return node_raw_features, edge_raw_features, full_data, train_data, val_data, test_data, cat_num
